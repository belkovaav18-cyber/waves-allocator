...
def manual_allocation_editor():
    ...
    with col3:
        if st.button("📊 Показать статистику"):
            ...
            st.dataframe(fix_dataframe_types(stats.head(20)))
...
with tab1:
    ...
    st.dataframe(fix_dataframe_types(residents[['ФИО', 'возраст', 'должность', 'город', 'comment']], width="stretch"))
...
with tab3:
    ...
    st.dataframe(fix_dataframe_types(guests_df, width="stretch"))
...
if st.session_state.final_result_df is not None and st.session_state.layout:
    ...
    st.dataframe(fix_dataframe_types(st.session_state.final_result_df[existing_cols], width="stretch"))
